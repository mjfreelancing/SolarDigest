...
namespace SolarDigest.Deploy.Schema
{
    ...
    {
        [SchemaTypeRequired]
        [LambdaDataSource(Shared.Constants.AppName, Constants.Function.GetSite)]
        ISite Site([SchemaTypeRequired] string id);

        [SchemaTypeRequired]
        [LambdaDataSource(Shared.Constants.AppName, Constants.Function.GetUploadUrl)]
        string UploadUrl([SchemaTypeRequired] IUploadUrlInput input);

        [SchemaTypeRequired]
        [LambdaDataSource(Shared.Constants.AppName, Constants.Function.GetUploadMultiPartUrls)]
        IUploadMultiParts UploadMultiPartUrls([SchemaTypeRequired] IUploadMultiPartInput input);

        [SchemaTypeRequired]
        [LambdaDataSource(Shared.Constants.AppName, Constants.Function.GetUploadMultiPartAbort)]
        bool UploadMultiPartAbort([SchemaTypeRequired] IUploadMultiPartAbortInput input);

        [SchemaTypeRequired]
        [LambdaDataSource(Shared.Constants.AppName, Constants.Function.GetUploadMultiPartComplete)]
        bool UploadMultiPartComplete([SchemaTypeRequired] IUploadMultiPartCompleteInput input);

        [SchemaTypeRequired]
        [LambdaDataSource(Shared.Constants.AppName, Constants.Function.GetDownloadUrl)]
